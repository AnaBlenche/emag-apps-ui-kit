...
(function($, window, document, undefined) {
    ...
    for (var attrName in extension) {
      if (typeof extension[attrName] === "function") {
        if (typeof tree[attrName] === "function") {
          ...
          tree[attrName] = _makeVirtualFunction(attrName, tree, base,
            extension, extName);
        } else if (attrName.charAt(0) === "_") {
          // Create private methods in tree.ext.EXTENSION namespace
          tree.ext[extName][attrName] = _makeVirtualFunction(attrName, tree,
            base, extension, extName);
        } else {
          $.error("Could not override tree." + attrName +
  ...
  var i, attr,
    ...
    NODE_ATTRS =
    "expanded extraClasses folder hideCheckbox icon key lazy refKey selected statusNodeType title tooltip unselectable"
    .split(" "),
    NODE_ATTR_MAP = {},
  ...
  FancytreeNode.prototype = /** @lends FancytreeNode# */ {
    ...
    _setChildren: function(children) {
      _assert(children && (!this.children || this.children.length === 0),
        "only init supported");
      this.children = [];
    ...
    addChildren: function(children, insertBefore) {
      ...
      if (insertBefore == null) {
        ...
        this.children.splice.apply(this.children, [pos, 0].concat(
          nodeList));
      }
    ...
    fixSelection3FromEndNodes: function() {
      ...
      this.visitParents(function(node) {
        ...
        state = allSelected ? true : (someSelected ? undefined :
          false);
        node._changeSelectStatusAttrs(state);
      });
    },
    ...
    moveTo: function(targetNode, mode, map) {
      ...
      if (!targetParent.isDescendantOf(prevParent) && targetParent !==
        prevParent) {
        targetParent.render();
      }
    ...
    replaceWith: function(source) {
      ...
      res.done(function(data) {
        ...
        parent.children.splice.apply(parent.children, [pos + 1, 0].concat(
          children));
    ...
    scrollIntoView: function(effects, options) {
      ...
      if (isParentWindow) {
        ...
        topNodeY = (topNode && topNode.span) ? $(topNode.span).offset().top :
          0;
        ...
        topNodeY = topNode ? $(topNode.span).offset().top -
          containerOffsetTop + scrollTop : 0;
        horzScrollbarHeight = Math.max(0, ($container.innerHeight() -
      ...
      if (nodeY < (scrollTop + topOfs)) {
        ...
        if (topNode) {
          _assert(topNode.isRootNode() || $(topNode.span).is(":visible"),
    ...
    setStatus: function(status, message, details) {
      return this.tree._callHook("nodeSetStatus", this, status, message,
        details);
    },
    /**Rename this node.
     * @param {string} title
        ...
        if (!message) {
          if (isMissing || required) {
            message = "'" + thisName + "' extension requires '" + name +
              "'";
            if (badOrder) {
              message += " to be registered " + (before ? "before" :
                "after") + " itself";
            }
          } else {
            message = "If used together, `" + name +
              "` must be registered " + (before ? "before" : "after") +
              " `" + thisName + "`";
          }
        }
        $.error(message);
   ...
   */
    ...
    findNextNode: function(match, startNode, visibleOnly) {
      var stopNode = null,
        ...
        walkVisible = function(parent, idx, fn) {
          ...
          if (grandParent) {
            return walkVisible(grandParent, grandParent.children.indexOf(
              parent) + 1, fn);
          } else {
      ...
      match = (typeof match === "string") ? _makeNodeTitleStartMatcher(
        match) : match;
      startNode = startNode || this.getFirstChild();

      walkVisible(startNode.parent, parentChildren.indexOf(startNode),
        function(node) {
    ...
    generateFormElements: function(selected, active, opts) {
      ...
      var nodeList,
        selectedName = (typeof selected === "string") ? selected : "ft_" +
        this._id + "[]",
        activeName = (typeof active === "string") ? active : "ft_" + this
        ._id + "_active",
        id = "fancytree_result_" + this._id,
        $result = $("#" + id),
        stopOnParents = this.options.selectMode === 3 && opts.stopOnParents !==
        false;
    ...
    loadKeyPath: function(keyPathList, callback, _rootNode) {
      ...
      for (i = 0; i < keyPathList.length; i++) {
        ...
        while (segList.length) {
          ...
          if (!node) {
            this.warn("loadKeyPath: key not found: " + key + " (parent: " +
              root + ")");
      ...
      function __lazyload(key, node, dfd) {
        ...
        node.load().done(function() {
          self.loadKeyPath.call(self, loadMap[key], callback, node).always(
            _makeResolveFunc(dfd, self));
        }).fail(function(errMsg) {
          self.warn("loadKeyPath: error loading: " + key +
            " (parent: " + root + ")");
  ...
  $.extend(Fancytree.prototype,
    /** @lends Fancytree_Hooks# */
    {
      ...
      nodeDblclick: function(ctx) {
        // TODO: return promise?
        if (ctx.targetType === "title" && ctx.options.clickFolderMode ===
          4) {
          //			this.nodeSetFocus(ctx);
          //			this._callHook("nodeSetActive", ctx, true);
          this._callHook("nodeToggleExpanded", ctx);
      ...
      nodeKeydown: function(ctx) {
        ...
        var matchNode, stamp, res, focusNode,
          ...
          clean = !(event.altKey || event.ctrlKey || event.metaKey ||
            event.shiftKey),
          $target = $(event.target),
          handled = true,
        ...
        if (opts.quicksearch && clean && /\w/.test(whichChar) && !$target
          .is(":input:enabled")) {
      ...
      nodeLoadChildren: function(ctx, source) {
        ...
        if (source.url) {
          if (node._requestId) {
            node.warn("Recursive load request #" + requestId + " while #" +
          ...
          if (ajax.debugDelay) {
            ...
            node.debug("nodeLoadChildren waiting debug delay " + Math.round(
              delay) + "ms");
          ...
          dfd.done(function(data, textStatus, jqXHR) {
            ...
            if ((this.dataType === "json" || this.dataType ===
                "jsonp") && typeof data === "string") {
              $.error(
            ...
            } else if (data && data.hasOwnProperty("d") && ctx.options
              .enableAspx) {
          ...
          source.done(function(children) {
            ...
            if (error === RECURSIVE_REQUEST_ERROR) {
              node.warn(
                ...
                requestId + " (expected #" + node._requestId + ")");
              return;
            ...
            if (tree._triggerNodeEvent("loadError", ctxErr, null) !==
              false) {
              tree.nodeSetStatus(ctx, "error", ctxErr.message, ctxErr
                .details);
            }
          });
        }
      ...
      nodeRemoveChild: function(ctx, childNode) {
        ...
        if (this.activeNode && (childNode === this.activeNode || this.activeNode
            .isDescendantOf(childNode))) {
          this.activeNode.setActive(false); // TODO: don't fire events
        }
        if (this.focusNode && (childNode === this.focusNode || this.focusNode
            .isDescendantOf(childNode))) {
          this.focusNode = null;
        }
        ...
        if (!isRootNode) {
          ...
          if (node.li && (force || (node.li.parentNode !== node.parent.ul))) {
            if (node.li.parentNode === node.parent.ul) {
              ...
              this.debug("Unlinking " + node + " (must be child of " +
                node.parent + ")");
            }
          ...
          if (isRootNode || node.expanded || deep === true) {
            ...
            while (childLI) {
              ...
              if (childNode2 && childNode2.parent !== node) {
                node.debug("_fixParent: remove missing " + childNode2,
                  childLI);
        ...
        if (icon !== false) {
          ...
          if (typeof icon === "string") {
            if (TEST_IMG.test(icon)) {
              ...
              icon = (icon.charAt(0) === "/") ? icon : ((opts.imagePath ||
                "") + icon);
              ares.push("<img src='" + icon +
                "' class='fancytree-icon' alt='' />");
            } else {
              ares.push("<span " + role +
                " class='fancytree-custom-icon " + icon + "'></span>");
            }
          } else {
      ...
      nodeSetActive: function(ctx, flag, callOpts) {
        ...
        } else if (flag && !noEvents && this._triggerNodeEvent(
        ...
        if (flag) {
          if (tree.activeNode) {
            _assert(tree.activeNode !== node,
              "node was active (inconsistency)");
            subCtx = $.extend({}, ctx, {
              node: tree.activeNode
            });
            tree.nodeSetActive(subCtx, false);
            _assert(tree.activeNode === null,
      ...
      nodeSetExpanded: function(ctx, flag, callOpts) {
        ...
        } else if (!noEvents && this._triggerNodeEvent("beforeExpand",
            node, ctx.originalEvent) === false) {
          // Callback returned false
          return _getRejectedPromise(node, ["rejected"]);
        }
        ...
        dfd.done(function() {
          ...
          if (flag && opts.autoScroll && !noAnimation && lastChild) {
            ...
            lastChild.scrollIntoView(true, {
              ...
              if (!noEvents) {
                ctx.tree._triggerNodeEvent(flag ? "expand" :
                  "collapse", ctx);
              }
            });
          } else {
            if (!noEvents) {
              ctx.tree._triggerNodeEvent(flag ? "expand" : "collapse",
                ctx);
            }
          }
        });
        ...
        _afterLoad = function(callback) {
          ...
          if (node.ul) {
            ...
            if (isVisible === isExpanded) {
              ...
              node.ul.style.display = (node.expanded || !parent) ? "" :
                "none";
              ...
              $(node.ul).toggle(effect.effect, effect.options, effect.duration,
                function() {
      ...
      nodeSetSelected: function(ctx, flag) {
        ...
        } else if (this._triggerNodeEvent("beforeSelect", node, ctx.originalEvent) ===
          false) {
          return !!node.selected;
        }
  ...
  $.widget("ui.fancytree",
    /** @lends Fancytree_Widget# */
    {
      ...
      _create: function() {
        ...
        this.$source = this.source || this.element.data("type") ===
          "json" ? this.element : this.element.find(">ul:first");
        ...
        for (i = 0; i < extensions.length; i++) {
          ...
          if (!extension) {
            $.error("Could not apply extension '" + extName +
              "' (it is not registered, did you forget to include it?)"
            );
          }
          // Add extension options as tree.options.EXTENSION
          //			_assert(!this.tree.options[extName], "Extension name must not exist as option name: " + extName);
          this.tree.options[extName] = $.extend(true, {}, extension.options,
            this.tree.options[extName]);
          // Add a namespace tree.ext.EXTENSION, to hold instance data
          _assert(this.tree.ext[extName] === undefined,
      ...
      _setOption: function(key, value) {
        ...
        this.tree.debug("set option " + key + "=" + value + " <" + typeof(
          value) + ">");
        if (callDefault) {
      ...
      _bind: function() {
        ...
        tree.$container.on("focusin" + ns + " focusout" + ns, function(
          event) {
          ...
          try {
            ...
            switch (event.type) {
              case "click":
                ...
                if (node.isPagingNode()) {
                  return tree._triggerNodeEvent("clickPaging", ctx,
                    event) === true;
                }
                return (tree._triggerNodeEvent("click", ctx, event) ===
                  false) ? false : tree._callHook("nodeClick", ctx);
              case "dblclick":
                ctx.targetType = et.type;
                return (tree._triggerNodeEvent("dblclick", ctx, event) ===
                  false) ? false : tree._callHook("nodeDblclick",
                  ctx);
            }
            //             } catch(e) {
    ...
    /** @lends Fancytree_Static# */
    {
      ...
      fixPositionOptions: function(opts) {
        ...
        if (!$.ui.fancytree.jquerySupports.positionMyOfs) {
          var // parse 'left+3 center' into ['left+3 center', 'left', '+3', 'center', undefined]
            ...
            // convert to numbers
            dx = (myParts[2] ? (+myParts[2]) : 0) + (atParts[2] ? (+
              atParts[2]) : 0),
            dy = (myParts[4] ? (+myParts[4]) : 0) + (atParts[4] ? (+
              atParts[4]) : 0);
      ...
      getEventTarget: function(event) {
        ...
        if (/\bfancytree-title\b/.test(tcn)) {
          ...
          res.type = (res.node.hasChildren() === false ? "prefix" :
            "expander");
      ...
      info: function(msg) {
        /*jshint expr:true */
        ($.ui.fancytree.debugLevel >= 1) && consoleApply("info",
          arguments);
      },
      ...
      registerExtension: function(definition) {
        _assert(definition.name != null,
          "extensions must have a `name` property.");
        _assert(definition.version != null,
