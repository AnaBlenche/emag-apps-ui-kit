  ...
  function addOffsetTopValue(stickElement) {
    ...
    if (stickElement.posTop) {
      $stickElement.stick_in_parent({
        'offset_top': offsetTop + stickElement.posTop - parseInt($stickElement.css("marginTop")),
        'recalc_every': RECALCULATION_INTERVAL
      }); //if RECALCULATION_INTERVAL = 1, the unstick event will not be triggered
    } else {
      //has no parent
      $stickElement.parent().addClass('stick-with-no-parent');
      $stickElement.stick_in_parent({
        'offset_top': offsetTop - parseInt($stickElement.css("marginTop")),
        'recalc_every': RECALCULATION_INTERVAL
      }); //if RECALCULATION_INTERVAL = 1, the unstick event will not be triggered
    }
  }
